...
You'll learn how to use Kessel to allow customers to organize and share documents in a Google Drive-like application.
Along the way, you'll get introduced to the following concepts:

- [**Resources and reporter representations**](../../building-with-kessel/concepts/resources-representations): The basic building block for leveraging Kessel's access control and inventory capabilities.
- [**Schema**](../../building-with-kessel/concepts/schema): The language for describing resources and their relationships.
- [**RBAC**](../../building-with-kessel/concepts/rbac): The access control and tenancy model for Kessel.
- [**Reporting resources**](../../building-with-kessel/how-to/report-resources): The process of telling Kessel about a resource and its relationships.
- [**Protecting resource access**](../../building-with-kessel/how-to/protect-endpoint): How to protect access to a resource.
...
<Steps>
1. **First we have to tell Kessel about the resource type.**
   In order to insert rows into a database, you have to define the tables and their columns.
   In Kessel, it is the same thing. Instead of a table, you have a resource type.
   Instead of columns, you have JSON-like attributes and the relationships they imply.
   Relationships can be used for access control.

   **In Kessel, a single resource can have multiple <i>representations</i> shared by different <i>reporters</i>.**
   For this tutorial, we'll define a single resource type called "document" which will be shared by a single reporter called "drive".
   ...
   The metadata for resources and reporters follows a directory structure.
    ...
    The document resource type has a single a 'drive' reporter.
    ...
    Here we configure the common representation schema. It includes a required attribute called "workspace_id".

    A <i>workspace</i> is a resource built-in to Kessel that groups other resources (including workspaces) together.
    This can be use to construct a resource hierarchy, common to many SaaS applications.
    In this tutorial, we use this instead of Drive folders.

    The workspace is in the common representation, because a document is always in a single workspace,
    ...
    We are working on a new schema language that will unify attributes and permissions (relationships) into a single language and fewer files.
    </Aside>
...
<Tabs>
  <TabItem label="Docker compose">
    <Steps>
    1. **Start Kessel Inventory**. Inventory is the service that stores resources, relationships, and handles access control.
        ...
        <Aside type="note">
        The local Docker setup runs with `allow-all` authorization — permission checks will always return allowed without
        requiring SpiceDB. Resource reporting and checking work end-to-end, but the RBAC setup scripts (create role / create
        role binding) require the Relations API gRPC service, which is being merged into Inventory.
        For full end-to-end authorization testing, use the **bonfire** tab.
        </Aside>
    </Steps>
  ...
  <TabItem label="bonfire">
    ...
    For information on ephemeral cluster access and setup, see the [bonfire documentation](https://github.com/RedHatInsights/bonfire).
    </Aside>
    <Steps>
...
|------------------------------|---------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------|
| **Schema Language**          | Resource attributes (JSON config) and permissions (KSL) use separate files                  | Unified schema language that combines attributes and permissions — no duplication                  |
| **Configuration Method**     | Edit schema and config via files                                                            | Configure everything at runtime via an API                                                         |
| **RBAC Management**          | Use temporary scripts or the pre-Kessel RBAC v1 API                                         | RBAC management API (v2) to encapsulate access grants and workspace management                    |
| **Services to Run**          | Relations API merger into Inventory is in progress                                           | Just run Inventory and SpiceDB — single service for resources, relationships, and access control   |
