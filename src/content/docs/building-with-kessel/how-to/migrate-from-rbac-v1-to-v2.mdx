...
The following sections describe two approaches to enable authentication with Kessel in the ephemeral environment.

#### Option 1: Using local Keycloak (Recommended for testing)

You can create a service account in the local Keycloak instance running in your ephemeral environment. This approach is simpler and doesn't require access to Stage SSO.

To create and retrieve the service account credentials for configuring your application, run the following commands:
...
1. Get an admin access token:
	...
	:::note
	The admin token expires quickly (typically within a few minutes). If you're executing these commands manually and encounter authentication errors, re-run this command to refresh the token.
	:::
...
1. Retrieve the service account client secret:
...
1. Get the subject (`sub` claim) for the service account:

	```shell
	SUB=$(curl -s ${ISSUER_URL}/protocol/openid-connect/token -d grant_type=client_credentials -d client_id=${CLIENT_ID} -d client_secret=${CLIENT_SECRET} | jq -r '.access_token' | awk -F. '{print $2}' | base64 --decode 2>/dev/null | jq -r '.sub')
	```

1. Configure RBAC to grant unlimited access to the service account:
...
1. Enable token validation in RBAC:
...
1. Update the Kessel Inventory service configuration to enable authentication.
   Run `oc edit cm inventory-api-config` to edit the kessel-inventory configuration ConfigMap and in the editor update the authn section like this:
	...
	Note: Replace `${ISSUER_URL}` and `${CLIENT_ID}` with the actual values.

1. Restart the RBAC and Kessel services to apply the changes:
...
Use these credentials to configure your application to authenticate with Kessel and RBAC.

#### Option 2: Using Stage SSO credentials

Alternatively, you can use service account credentials from Stage SSO. This approach is useful if you want to test with real Stage SSO authentication:
